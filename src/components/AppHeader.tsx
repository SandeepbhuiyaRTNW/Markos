...
// Darkest-region contrast including grain: ink 15.867:1, muted 6.305:1.
const INK = '#14100e';
const MUTED = '#5c534b';
...
export default function AppHeader({ mode = 'full', active = null, email, onHome, onSessions, onTalk, onWrite, onSettings, onClose }: AppHeaderProps) {
  ...
  const item = (label: string, key: NavKey, onClick?: () => void) => (
    <button
      onClick={onClick}
      className="transition-opacity hover:underline underline-offset-4"
  ...
  return (
    <header className="relative z-20 flex-none">
      <div className="flex items-center justify-between px-6 sm:px-10 lg:px-16" style={{ height: 60 }}>
        <button onClick={onHome ?? onClose} style={WORDMARK} className="transition-opacity hover:underline underline-offset-4">Markos</button>
        {mode === 'focused' ? (
          <button onClick={onClose} className="transition-opacity hover:underline underline-offset-4" style={{ fontSize: 13, letterSpacing: '0.04em', color: MUTED }}>Close</button>
        ) : (
